...
st.markdown("""
    ...
    html, body, [class*="css"] { font-family: 'Libre Baskerville', serif; color: #2c3e50; }
    ...
    /* This makes your photographs look beautiful */
    .article-card img {
        display: block;
        margin-left: auto;
        margin-right: auto;
...
def main():
    ...
    if choice == "🏠 Archive":
        ...
        if not search_query:
            st.markdown(f"""<div class="mission-container"><p style="font-size: 1.1rem; text-align: center; font-style: italic;">"At <b>The English Archive</b>, we believe that the pursuit of literature belongs to everyone. Education is a right, not a luxury."</p></div><br>""", unsafe_allow_html=True)

        query = "SELECT * FROM articles WHERE status='Published' ORDER BY id DESC"
        if search_query:
            query = f"SELECT * FROM articles WHERE status='Published' AND (title LIKE '%{search_query}%' OR content LIKE '%{search_query}%') ORDER BY id DESC"
        
        articles = pd.read_sql_query(query, conn)
        
        for index, row in articles.iterrows():
            # Use an f-string to wrap the database content in our card div
            st.markdown(f"""
                <div class="article-card">
                    ...
                    <h2 style="color: #1a5e63; margin-top: 5px; font-family: 'Montserrat';">{row['title']}</h2>
                    <hr style="border: 0; border-top: 1px solid #eee; margin: 20px 0;">
                    <div class="article-body">{row['content']}</div>
                    <p style="margin-top: 20px; font-size: 0.8rem; color: #999;">Published: {row['date']}</p>
                </div>
            """, unsafe_allow_html=True)
