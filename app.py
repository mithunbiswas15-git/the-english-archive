...
# We use os.environ directly to avoid the StreamlitSecrets crash on Render
URL = os.environ.get("SUPABASE_URL")
KEY = os.environ.get("SUPABASE_KEY")

if not URL or not KEY:
    st.error("Database credentials not found. Please add SUPABASE_URL and SUPABASE_KEY to the Environment tab in Render.")
    st.stop()

supabase: Client = create_client(URL, KEY)

# --- 2. ADVANCED TURBO CACHING ---
# Keeps the gallery in RAM for 1 hour (3600s) for lightning speed for students
...
def main():
    ...
    if choice == "🏠 Archive":
        if st.session_state.viewing_id:
            ...
            # Fetching from the hour-long cache
        ...
        if not st.session_state.logged_in:
            ...
            if admin_opt == "Draft New":
                ...
                if st.button("Archive Manuscript"):
                    ...
                    # Force clear cache so students see the new article immediately
                    st.cache_data.clear() 
